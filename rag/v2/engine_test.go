...
// MockLLM implements iface.LLM for testing
type MockLLM struct {
	ChatResponse string
	Embedding    []float32
}
...
func (m *MockLLM) ListModels(ctx context.Context) ([]*models.Model, error) {
	return nil, nil
}

func (m *MockLLM) Generate(ctx context.Context, r *models.GenerateRequest) (*models.GenerateResponse, error) {
	return &models.GenerateResponse{Text: m.ChatResponse}, nil
}
...
func (s *EngineTestSuite) TestFullRAGFlow() {
	...
	// 1. Setup Components
	// Mock LLM for both Embed and Chat
	mockLLM := &MockLLM{
		Embedding:    []float32{0.1, 0.2, 0.3},
		ChatResponse: "This is a generated answer based on context.",
	}
	...
	retriever := NewVectorRetriever(vectorStore, mockLLM, "test-embed-model", 1) // TopK=1
	synthesizer := NewSimpleSynthesizer(mockLLM, "test-llm-model")
...
func (s *EngineTestSuite) TestFullRAGFlow_Chromem() {
	...
	retriever := NewVectorRetriever(chromemStore, mockLLM, "test-embed-model", 1) // TopK=1
	synthesizer := NewSimpleSynthesizer(mockLLM, "test-llm-model")
